...
hyperparameter_defaults = dict(
    seed=0,
    dataset_name="snRNA_ocular_surface_ncbi_kan",
...
if dataset_name == "snRNA_ocular_surface_ncbi_kan":
    data_dir = Path("/root/autodl-tmp/hzdata/scdata")
    adata = sc.read(data_dir / "snRNA2000_ocular_surface.h5ad")
    adata_test = sc.read(data_dir / "snRNA500_ocular_surface.h5ad")
    # adata_teq = sc.read(data_dir / "optic4_200cells.h5ad")

    # import anndata
    # import numpy as np
    # # 第一步：随机抽取2000个细胞
    # idx_2000 = np.random.choice(adata.n_obs, 2000, replace=False)
    # adata_2000 = adata[idx_2000, :]
    # adata_2000.write(data_dir/'opt3_sub2000_random.h5ad')

    # # 剩下的索引
    # all_idx = np.arange(adata.n_obs)
    # remain_idx = np.setdiff1d(all_idx, idx_2000)

    # # 第二步：从剩下的细胞中随机选1000个
    # idx_500 = np.random.choice(remain_idx, 500, replace=False)
    # adata_500 = adata[idx_500, :]
    # adata_500.write(data_dir/'opt3_sub500_random.h5ad')
