...
# def plot_value_distributions(train_data, valid_data, mask_value, pad_value):
#     # Flatten并去除mask和pad
#     # ###Masked values 分布直方图（训练与验证对比）
#     train_values = train_data['values'].cpu().numpy().flatten()
#     valid_values = valid_data['values'].cpu().numpy().flatten()
#     train_values = train_values[(train_values != mask_value) & (train_values != pad_value)]
#     valid_values = valid_values[(valid_values != mask_value) & (valid_values != pad_value)]

#     plt.hist(train_values, bins=50, alpha=0.5, label='Train', density=True)
#     plt.hist(valid_values, bins=50, alpha=0.5, label='Valid', density=True)
#     plt.xlabel('Value')
#     plt.ylabel('Density')
#     plt.title('Values Distribution (Masked)')
#     plt.legend()
#     plt.savefig('value_distribution_masked.png', dpi=300)
    

# import pandas as pd
# import seaborn as sns

# def plot_boxplot_by_batch(data, mask_value, pad_value, name='train'):
#     #按 batch 分组的箱线图
#     values = data['values'].cpu().numpy()
#     batches = data['batch_labels'].cpu().numpy()
#     flat_values = values.flatten()
#     flat_batches = np.repeat(batches, values.shape[1])
#     mask = (flat_values != mask_value) & (flat_values != pad_value)

#     df = pd.DataFrame({'value': flat_values[mask], 'batch': flat_batches[mask]})
#     plt.figure(figsize=(10,6))
#     sns.boxplot(x='batch', y='value', data=df)
#     plt.title(f'Values by Batch - {name}')
#     plt.savefig(f'values_by_batch_{name}.png', dpi=300)

# def plot_violin_by_class(data, mask_value, pad_value, name='train'):
#     #  类别标签小提琴图
#     values = data['values'].cpu().numpy()
#     classes = data['class_labels'].cpu().numpy()
#     flat_values = values.flatten()
#     flat_classes = np.repeat(classes, values.shape[1])
#     mask = (flat_values != mask_value) & (flat_values != pad_value)

#     df = pd.DataFrame({'value': flat_values[mask], 'class': flat_classes[mask]})
#     plt.figure(figsize=(12,6))
#     sns.violinplot(x='class', y='value', data=df, cut=0)
#     plt.title(f'Values by Class - {name}')
#     plt.savefig(f'values_by_class_{name}.png', dpi=300)

# def plot_input_vs_target(data, mask_value, pad_value, name='train'):
#     #输入和目标的散点对比
#     input_values = data['values'].cpu().numpy().flatten()
#     target_values = data['target_values'].cpu().numpy().flatten()
#     mask = (input_values != mask_value) & (input_values != pad_value)
#     plt.scatter(input_values[mask], target_values[mask], alpha=0.2, s=1)
#     plt.xlabel('Input Value')
#     plt.ylabel('Target Value')
#     plt.title(f'Input vs Target - {name}')
#     plt.savefig(f'input_vs_target_{name}.png', dpi=300)


# def plot_label_counts(data, name='train'):
#     #batch 和 class 的数量统计条形图
#     batch_counts = pd.Series(data['batch_labels'].cpu().numpy()).value_counts().sort_index()
#     class_counts = pd.Series(data['class_labels'].cpu().numpy()).value_counts().sort_index()
#     plt.figure(figsize=(12,4))
#     plt.subplot(1,2,1)
#     batch_counts.plot(kind='bar')
#     plt.title(f'Batch Label Counts - {name}')
#     plt.xlabel('Batch')
#     plt.ylabel('Count')
#     plt.subplot(1,2,2)
#     class_counts.plot(kind='bar', color='orange')
#     plt.title(f'Class Label Counts - {name}')
#     plt.xlabel('Class')
#     plt.ylabel('Count')
#     plt.tight_layout()
#     plt.savefig(f'label_counts_{name}.png', dpi=300)
