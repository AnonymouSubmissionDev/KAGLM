...
import NCBI_KAN as ncbi
from NCBI_KAN.model import TransformerModel, AdversarialDiscriminator
from NCBI_KAN.tokenizer import tokenize_and_pad_batch, random_mask_value
from NCBI_KAN.loss import (
    masked_mse_loss,
    masked_relative_error,
    criterion_neg_log_bernoulli,
)
from NCBI_KAN.tokenizer.gene_tokenizer import GeneVocab
from NCBI_KAN.preprocess import Preprocessor
from NCBI_KAN import SubsetsBatchSampler
from NCBI_KAN.utils import set_seed, category_str2int, eval_scib_metrics
...
logger = ncbi.logger
ncbi.utils.add_file_handler(logger, save_dir / "run.log")

# if dataset_name == "LBCL_NCBI_KAN":
#     data_dir = Path("/root/autodl-tmp/hzdata/scdata")
#     adata = sc.read(data_dir / "f5b09167-e4b5-4f32-b7b4-f0b7c402a4c4.h5ad")
#     # adata_test = sc.read(data_dir / "opt3_sub500_random.h5ad")
#     # adata_teq = sc.read(data_dir / "optic4_200cells.h5ad")

#     import anndata
#     import numpy as np
#     # 第一步：随机抽取2000个细胞
#     idx_3000 = np.random.choice(adata.n_obs, 3000, replace=False)
#     adata_3000 = adata[idx_3000, :]
#     adata_3000.write(data_dir/'LBCL_LME_sub3000_random.h5ad')

#     # 剩下的索引
#     all_idx = np.arange(adata.n_obs)
#     remain_idx = np.setdiff1d(all_idx, idx_3000)

#     # 第二步：从剩下的细胞中随机选1000个
#     idx_1000 = np.random.choice(remain_idx, 1000, replace=False)
#     adata_1000 = adata[idx_1000, :]
#     adata_1000.write(data_dir/'LBCL_LME_sub1000_random.h5ad')
if dataset_name == "LBCL_NCBI_KAN":
    data_dir = Path("/root/autodl-tmp/hzdata/scdata")
    # adata = sc.read(data_dir / "f5b09167-e4b5-4f32-b7b4-f0b7c402a4c4.h5ad")
    adata = sc.read(data_dir / "LBCL_LME_sub3000_random.h5ad")
    adata_test = sc.read(data_dir / "LBCL_LME_sub1000_random.h5ad")

    
    adata.obs["celltype"] = adata.obs["cell_type"].astype("category")
    adata_test.obs["celltype"] = adata_test.obs["cell_type"].astype("category")
    adata.obs["str_batch"] = adata.obs["tissue_ontology_term_id"].astype(str)
    adata_test.obs["str_batch"] = adata.obs["tissue_ontology_term_id"].astype(str)
    adata.obs["batch_id"]  = adata.obs["str_batch"] = "0"
    adata_test.obs["batch_id"]  = adata_test.obs["str_batch"] = "1"
    adata.var["gene_name"] = adata.var["feature_name"]
    adata_test.var["gene_name"] = adata.var["feature_name"]
    adata.var.set_index(adata.var["gene_name"], inplace=True)
    adata_test.var.set_index(adata.var["gene_name"], inplace=True)
    print(np.min(adata_test.X), np.max(adata_test.X))###判断数据是不是raw
    # 统计数据的分布可以帮助判断数据是否经过处理。
    # 原始计数矩阵 的值通常是整数，并且分布范围较宽（如 0 到上千甚至更高）。
    # 归一化矩阵 的值通常是浮点数，范围较小（如 0 到 20 或更低）。
    # 对数变换 后的数据通常范围更小，且不会出现大值。
    data_is_raw = False
...
def evaluate(model: nn.Module, loader: DataLoader, return_raw: bool = False) -> float:
    ...
    return total_loss / total_num, total_error / total_num, np.concatenate(predictions, axis=0),np.concatenate(GTS, axis=0)
...
for epoch in range(1, epochs + 1):
    ...
    val_loss, val_err,predictions,GTS = evaluate(
        model,
        loader=valid_loader,
    )
    ...
    precision = precision_score(GTS, predictions, average="macro")
    recall = recall_score(GTS, predictions, average="macro")
    macro_f1 = f1_score(GTS, predictions, average="macro")#
    ####
    report = classification_report(GTS, predictions, target_names=list(id2type.values()),
                                   labels=list(id2type.keys()))
    class_report = classification_report(GTS, predictions, target_names=list(id2type.values()),
...
torch.save(best_model.state_dict(), save_dir / "last.pt")